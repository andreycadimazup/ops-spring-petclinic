 ...
 */
package org.springframework.samples.petclinic.owner.rest.mappers;
...
@Component
public class RestOwnerMapper {

	public OwnerSummaryResponse toOwnerSummary(Owner owner) {
	...
	public PetResponse toPetResponse(Integer ownerId, Pet pet) {
	...
	public PetTypeResponse toPetTypeResponse(PetType petType) {
		return new PetTypeResponse(petType.getId(), petType.getName());
	}

	public VisitResponse toVisitResponse(Integer petId, Visit visit) {
		return new VisitResponse(visit.getId(), petId, visit.getDate(), visit.getDescription());
	}

	public List<PetResponse> toPetResponses(Owner owner) {
